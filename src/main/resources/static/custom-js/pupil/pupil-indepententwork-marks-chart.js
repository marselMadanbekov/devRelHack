$(document).ready(function () {
    const independentChartCanvas = document.getElementById('pupils-independent-work-mark-chart');
    const prevMonthButton = document.getElementById('prevWeekButtonIndependentWork');
    const nextMonthButton = document.getElementById('nextWeekButtonIndependentWork');
    const pupilId = document.getElementById('pupilId').value;
    const subjectId = document.getElementById("subjectId").value;
    let weekShift = 0;
    let independentChart = null;
    ...
    function updateChart() {
        if (independentChart) {
            independentChart.destroy();
        }

        $.ajax({
            url: '/pupils/statistics/independent-work-statistic?pupilId=' + pupilId + '&subjectId=' + subjectId + '&weekShift=' + weekShift,
            method: 'GET',
            dataType: 'json',
            success: function (data) {
                ...
                independentChart = new Chart(independentChartCanvas, {
                    type: 'bar',
                    data: chartData,
                    options: {
                        responsive: true,
