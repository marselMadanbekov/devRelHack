...
public class Plus8 implements TaskGenerator {
    private Plus8(){};

    private static class SingletonHolder{
        public static final Plus8 HOLDER_INSTANCE = new Plus8();
    }
    public static Plus8 getInstance(){
        return Plus8.SingletonHolder.HOLDER_INSTANCE;
    }
    @Override
    public int head(int sum, int leftNum) {
        switch (sum) {
            ...
            case 2:
                if (random.nextInt(10) > 3) {
                    if (random.nextInt(10) > 5) return 8;
                    if (random.nextInt(10) > 4) return random.nextInt(2) + 8;
