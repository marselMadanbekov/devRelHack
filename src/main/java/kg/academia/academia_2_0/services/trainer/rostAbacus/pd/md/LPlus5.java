...
public class LPlus5 implements TaskGenerator {
    private LPlus5(){};

    private static class SingletonHolder{
        public static final LPlus5 HOLDER_INSTANCE = new LPlus5();
    }
    public static LPlus5 getInstance(){
        return LPlus5.SingletonHolder.HOLDER_INSTANCE;
    }
    @Override
    ...
    public int tail(int sum, int leftNum, boolean isPlus) {
        ...
        switch (sum) {
            ...
            case 8:
                if (isPlus) {
                    if(canAdd){
                        if(random.nextInt(10) > 5) return 5;
                        if(random.nextInt(10) > 5) return random.nextInt(5) + 5;
                    }
                    return 1;
